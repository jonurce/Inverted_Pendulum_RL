...
model_full = SAC.load("../Trained Models/sac_7.zip", device="cpu")
last_angle = 0.0
...
def control_system_RL_full(current_time, dt, motor_angle, pendulum_angle, rpm):
    ...
    if abs(pendulum_angle - last_angle) < 1e-6 or dt < 1e-6:
        d1 = 0.0
    else:
...
model_partial = SAC.load("../Trained Models/sac_8.zip", device="cpu")
n_frames = 1
