...
class QubeServo2Env(gym.Env):
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(5,), dtype=np.float32)  # Full state
        self.state = None
        self.dt = 0.01
        self.max_steps = 2000
        self.step_count = 0
    ...
    def step(self, action):
        ...
        self.d1 = new_full_state[-1]
        s0, c0, d0, s1, c1, d1 = new_full_state
...
model.save("Trained Models/sac_12")

# Test and collect data
env = QubeServo2Env()
obs, _ = env.reset()
frame_history = [obs] * n_frames
rewards = []
thetas = []
alphas = []
voltages = []
